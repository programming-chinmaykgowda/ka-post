...
$sql = ",SUM(Target_2023_24) AS Target_2023_24, SUM(SB_AC) AS SB_AC, SUM(RD_AC) AS RD_AC, SUM(TD_AC) AS TD_AC, SUM(SSA_AC) AS SSA_AC,SUM(MSSC_AC) AS MSSC_AC,SUM(PPF_AC) AS PPF_AC, SUM(MIS_AC) AS MIS_AC, SUM(SCSS_AC) AS SCSS_AC, SUM(KVP_AC) AS KVP_AC, SUM(NSC_AC) AS NSC_AC, SUM(TOTAL_AC_OPENED) AS TOTAL_AC_OPENED, (SUM(TOTAL_AC_OPENED)/SUM(Target_2023_24))*100 AS `%age`, Remarks,AO_Name,Sub_Division,Division FROM posb_karnataka_2022_23 WHERE 1";
        ...
        /*if (is_numeric($achievment)) {
            $sums["Achievment"] = 0;
		    		if($sums["Target_2023_24"]) {
		    			$sums["Achievment"] = $sums["TOTAL_AC_OPENED"] / $sums["Target_2023_24"];
		    		}
		    		$sums["Achievment"] *= 100;
