...
$sql = ", SUM(2021_22) AS 2021_22, SUM(mssc_23_24) AS mssc_23_24,    SUM(mssc_target) AS mssc_target,SUM(mssc_prop_target) AS mssc_prop_target,SUM(A_c_Opened) AS A_c_Opened,SUM(A_c_Closed) AS A_c_Closed,SUM(Net_Acs) AS Net_Acs,SUM(Cert_Issued) AS Cert_Issued,SUM(Cert_Disch) AS Cert_Disch,SUM(Net_Cert) AS Net_Cert,SUM(MSSC_Opened) AS MSSC_Opened,SUM(Msscclosed) AS Msscclosed,SUM(Net_mssc) AS Net_mssc,SUM(mssc_ac) AS mssc_ac,SUM(Total_closed) AS Total_closed,SUM(Total_Net) AS Total_Net,(SUM(mssc_ac)/SUM(mssc_target))*100 AS `%age`,(SUM(mssc_ac)/SUM(mssc_prop_target))*100 AS `%age1`, Remarks,AO_Name,Sub_Division,Division FROM posb_karnataka1_24_25 WHERE 1";
if($region !== "all") {
...
if ($result->num_rows > 0) {
    ...
    echo "<table border='2'>
         ...
         <tr class='sticky-row'>
           ...
           <th  data-backendName='mssc_23_24'>MSSC A/cs Opened </br> [2023-24]</th>
            <th data-backendName='mssc_target'>MSSC Target </br>[2024-25]</th>
            <th data-backendName='mssc_prop_target'>Proportion </br>Target</th>
           
            
           
           
             <th data-backendName='mssc_ac'>Total MSSC A/cs <br />opened </th>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>{$row["mssc_23_24"]}</td>"; 
        echo "<td>{$row["mssc_target"]}</td>";
        echo "<td>{$row["mssc_prop_target"]}</td>";
        ...
        if (is_numeric($row["Name"])) {
         ...
         if (is_numeric($row["mssc_23_24"])) {
            $sums["mssc_23_24"] += (int) $row["mssc_23_24"];
        }
        
        if (is_numeric($row["mssc_target"])) {
            $sums["mssc_target"] += (int) $row["mssc_target"];
        }
        if (is_numeric($row["mssc_prop_target"])) {
            $sums["mssc_prop_target"] += (int) $row["mssc_prop_target"];
        }
        ...
        if (is_numeric($achievment)) {
            $sums["Achievment"] = 0;
		    		if($sums["mssc_target"]) {
		    			$sums["Achievment"] = $sums["mssc_ac"] / $sums["mssc_target"];
		    		}
		    		$sums["Achievment"] *= 100;
        ...
        if (is_numeric($achievment1)) {
            $sums["Achievment1"] = 0;
		    		if($sums["mssc_prop_target"]) {
		    			$sums["Achievment1"] = $sums["mssc_ac"] / $sums["mssc_prop_target"];
		    		}
