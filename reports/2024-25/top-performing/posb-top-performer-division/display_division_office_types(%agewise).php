...
$sql = "SELECT Division, Office_Name, Sub_Division, A_c_Opened, onlyposb_achivement, Office_Type
        FROM (
            SELECT Division, Office_Name, Sub_Division, A_c_Opened, onlyposb_achivement, Office_Type,
                ROW_NUMBER() OVER(PARTITION BY Division, Office_Type ORDER BY onlyposb_achivement DESC) AS row_num
...
echo "<h2 style='text-align:center;'>POSB- Top 3 Head Offices, Sub Offices & Branch Offices in Each Division under SK Region(Percentage of posb_achivement wise)</h2>";
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($currentDivision != $division || $currentOfficeType != $officeType) {
        ...
        echo "<tr>
            <th>Office Name</th>
            <th>Sub Division</th>
            <th class='small-column'>Total POSB A/Cs</th>
    ...
    echo "<tr>
        ...
        <td class='small-column'>$onlyposb_achivement</td>
