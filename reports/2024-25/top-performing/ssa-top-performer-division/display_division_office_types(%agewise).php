...
$sql = "SELECT Division, Office_Name, Sub_Division, ssa_ac, ssa_Achievement, Office_Type
        FROM (
            SELECT Division, Office_Name, Sub_Division, ssa_ac, ssa_Achievement, Office_Type,
                ROW_NUMBER() OVER(PARTITION BY Division, Office_Type ORDER BY ssa_Achievement DESC) AS row_num
...
echo "<h2 style='text-align:center;'>SSA- Top 3 Head Offices, Sub Offices & Branch Offices in Each Division under SK Region(Percentage of SSA_achievement wise)</h2>";
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($currentDivision != $division || $currentOfficeType != $officeType) {
        ...
        echo "<tr>
            <th>Office Name</th>
            <th>Sub Division</th>
            <th class='small-column'>Total SSA A/Cs</th>
    ...
    echo "<tr>
        ...
        <td class='small-column'>$ssa_Achievement</td>
