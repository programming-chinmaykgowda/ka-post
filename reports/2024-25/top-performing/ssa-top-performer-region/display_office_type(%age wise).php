...
echo "<h2 style='text-align:center;'>SSA- Top 3 Departmental Offices & Branch Offices under SK Region (2024-25)</h2>";

foreach ($officeTypes as $officeType) {
    // Prepare and execute SQL query to retrieve the top three offices of the specified type in each region
    $sql = "SELECT Region, Office_Name, Division, ssa_ac, ssa_Achievement
            FROM (
                SELECT Region, Office_Name, Division, ssa_ac, ssa_Achievement,
                    ROW_NUMBER() OVER(PARTITION BY Region ORDER BY ssa_Achievement DESC) AS row_num
                FROM posb_karnataka1_24_25
                WHERE Office_Type = ?
            ) AS ranked
            WHERE row_num <= 3
            ORDER BY Region, ssa_Achievement DESC";
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        echo "<tr><th>Office Name</th><th>Division Name</th><th>Total SSA Accounts</th><th>ssa_Achievement</th><th>Position in Region</th></tr>";

        while ($row = $result->fetch_assoc()) {
            $region = $row['Region'];
            $officeName = $row['Office_Name'];
            $division = $row['Division'];
            $totalAccounts = $row['ssa_ac'];
            $ssa_Achievement = $row['ssa_Achievement'];
            ...
            // Display office data
            echo "<tr><td>$officeName</td><td>$division</td><td style='text-align:center;'>$totalAccounts</td><td>$ssa_Achievement</td><td style='text-align:center;'>{$serialNumbers[$officeType]}</td></tr>";
