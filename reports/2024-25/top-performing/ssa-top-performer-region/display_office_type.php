...
foreach ($officeTypes as $officeType) {
    ...
    $sql = "SELECT Region, Office_Name, Division, ssa_ac
            FROM (
                SELECT Region, Office_Name, Division, ssa_ac,
                    ROW_NUMBER() OVER(PARTITION BY Region ORDER BY ssa_ac DESC) AS row_num
                FROM posb_karnataka1_24_25
                WHERE Office_Type = '$officeType'
            ) AS ranked
            WHERE row_num <= 3
            ORDER BY Region, ssa_ac DESC";
    ...
    echo "<table>";
    echo "<tr><th>Office Name</th><th>Division Name</th><th>Total SSA Accounts</th><th>Position in Region</th></tr>";
