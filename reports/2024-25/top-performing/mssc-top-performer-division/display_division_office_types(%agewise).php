...
$sql = "SELECT Division, Office_Name, Sub_Division, mssc_ac, mssc_Achievement, Office_Type
        FROM (
            SELECT Division, Office_Name, Sub_Division, mssc_ac, mssc_Achievement, Office_Type,
                ROW_NUMBER() OVER(PARTITION BY Division, Office_Type ORDER BY mssc_Achievement DESC) AS row_num
...
echo "<h2 style='text-align:center;'>MSSC- Top 3 Head Offices, Sub Offices & Branch Offices in Each Division under SK Region(Percentage of MSSC_achievement wise)</h2>";
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($currentDivision != $division || $currentOfficeType != $officeType) {
        ...
        echo "<tr>
            <th>Office Name</th>
            <th>Sub Division</th>
            <th class='small-column'>Total MSSC A/Cs</th>
    ...
    echo "<tr>
        ...
        <td class='small-column'>$mssc_Achievement</td>
