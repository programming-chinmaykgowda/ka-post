...
$sql = ",SUM(all_posb_target) AS all_posb_target, SUM(all_posb_prop_target) AS all_posb_prop_target, SUM(Total_Acs) AS Total_Acs, SUM(SB_AC) AS SB_AC, SUM(RD_AC) AS RD_AC, SUM(TD_AC) AS TD_AC, SUM(ssa_ac) AS ssa_ac,SUM(mssc_ac) AS mssc_ac,SUM(PPF_AC) AS PPF_AC, SUM(MIS_AC) AS MIS_AC, SUM(SCSS_AC) AS SCSS_AC, SUM(KVP_AC) AS KVP_AC, SUM(NSC_AC) AS NSC_AC,  (SUM(Total_Acs)/SUM(all_posb_target))*100 AS `%age`,(SUM(Total_Acs)/SUM(all_posb_prop_target))*100 AS `%age1`, Remarks,AO_Name,Sub_Division,Division FROM posb_karnataka1_24_25 WHERE 1";
...
if ($result->num_rows > 0) {
    ...
    echo "<table border='2'>
       ...
       <tr class='sticky-row'> 
            ...
            <th data-backendName='all_posb_target'>Annual </br>Target</th>
            <th data-backendName='all_posb_prop_target'>Proportion </br>Target</th>
              <th data-backendName='Total_Acs'>Total A/cs </br> Opened</th>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>{$row["all_posb_target"]}</td>";
        echo "<td>{$row["all_posb_prop_target"]}</td>";
       ...
       // Update the sums
        ...
        if (is_numeric($row["all_posb_target"])) {
            $sums["all_posb_target"] += (int) $row["all_posb_target"];
        }
        if (is_numeric($row["all_posb_prop_target"])) {
            $sums["all_posb_prop_target"] += (int) $row["all_posb_prop_target"];
        }
        ...
        if (is_numeric($achievment)) {
            $sums["Achievment"] = 0;
		    		if($sums["all_posb_target"]) {
		    			$sums["Achievment"] = $sums["Total_Acs"] / $sums["all_posb_target"];
		    		}
		    		$sums["Achievment"] *= 100;
        ...
        if (is_numeric($achievment1)) {
            $sums["Achievment1"] = 0;
		    		if($sums["all_posb_prop_target"]) {
		    			$sums["Achievment1"] = $sums["Total_Acs"] / $sums["all_posb_prop_target"];
		    		}
