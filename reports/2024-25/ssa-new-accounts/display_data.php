...
$sql = ", SUM(ssa_22_23) AS ssa_22_23, SUM(ssa_23_24) AS ssa_23_24,   SUM(ssa_target) AS ssa_target,SUM(ssa_prop_target) AS ssa_prop_target,SUM(A_c_Opened) AS A_c_Opened,SUM(A_c_Closed) AS A_c_Closed,SUM(Net_Acs) AS Net_Acs,SUM(Cert_Issued) AS Cert_Issued,SUM(Cert_Disch) AS Cert_Disch,SUM(Net_Cert) AS Net_Cert,SUM(MSSC_Opened) AS MSSC_Opened,SUM(Msscclosed) AS Msscclosed,SUM(Net_mssc) AS Net_mssc,SUM(ssa_ac) AS ssa_ac,SUM(Total_closed) AS Total_closed,SUM(Total_Net) AS Total_Net,(SUM(ssa_ac)/SUM(ssa_target))*100 AS `%age`,(SUM(ssa_ac)/SUM(ssa_prop_target))*100 AS `%age1`, Remarks,AO_Name,Sub_Division,Division FROM posb_karnataka1_24_25 WHERE 1";
if($region !== "all") {
...
if ($result->num_rows > 0) {
    ...
    echo "<table border='2'>
         ...
         <tr class='sticky-row'>
            ...
            <th class='left-align' data-backendName='Name'>Name</th>".$addNames."
             <th  data-backendName='ssa_22_23'>SSA A/cs Opened </br> [2022-23]</th>
           <th  data-backendName='ssa_23_24'>SSA A/cs Opened </br> [2023-24]</th>
            <th data-backendName='ssa_target'>SSA Target </br>[2024-25]</th>
            <th data-backendName='ssa_prop_target'>Proportion </br>Target</th>
        
           
           
             <th data-backendName='ssa_ac'>Total SSA A/cs <br />opened </th>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if(!empty($aoname)) {
		 ...
		 echo "<td>{$row["ssa_22_23"]}</td>";     
        echo "<td>{$row["ssa_23_24"]}</td>"; 
        echo "<td>{$row["ssa_target"]}</td>";
        echo "<td>{$row["ssa_prop_target"]}</td>";
        ...
        if (is_numeric($row["ssa_22_23"])) {
            $sums["ssa_22_23"] += (int) $row["ssa_22_23"];
        }
        
         if (is_numeric($row["ssa_23_24"])) {
            $sums["ssa_23_24"] += (int) $row["ssa_23_24"];
        }
        
        if (is_numeric($row["ssa_target"])) {
            $sums["ssa_target"] += (int) $row["ssa_target"];
        }
        if (is_numeric($row["ssa_prop_target"])) {
            $sums["ssa_prop_target"] += (int) $row["ssa_prop_target"];
        }
        ...
        if (is_numeric($achievment)) {
            $sums["Achievment"] = 0;
		    		if($sums["ssa_target"]) {
		    			$sums["Achievment"] = $sums["ssa_ac"] / $sums["ssa_target"];
		    		}
		    		$sums["Achievment"] *= 100;
        ...
        if (is_numeric($achievment1)) {
            $sums["Achievment1"] = 0;
		    		if($sums["ssa_prop_target"]) {
		    			$sums["Achievment1"] = $sums["ssa_ac"] / $sums["ssa_prop_target"];
		    		}
