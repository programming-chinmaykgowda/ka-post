...
$sql = ",SUM(Target_2023_24) AS Target_2023_24, SUM(P_Target) AS P_Target, SUM(TOTAL_AC_OPENED) AS TOTAL_AC_OPENED, SUM(SB_AC) AS SB_AC, SUM(RD_AC) AS RD_AC, SUM(TD_AC) AS TD_AC, SUM(SSA_AC) AS SSA_AC,SUM(MSSC_AC) AS MSSC_AC,SUM(PPF_AC) AS PPF_AC, SUM(MIS_AC) AS MIS_AC, SUM(SCSS_AC) AS SCSS_AC, SUM(KVP_AC) AS KVP_AC, SUM(NSC_AC) AS NSC_AC,  (SUM(TOTAL_AC_OPENED)/SUM(Target_2023_24))*100 AS `%age`,(SUM(TOTAL_AC_OPENED)/SUM(P_Target))*100 AS `%age1`, Remarks,AO_Name,Sub_Division,Division FROM posb_karnataka_2023_24 WHERE 1";
