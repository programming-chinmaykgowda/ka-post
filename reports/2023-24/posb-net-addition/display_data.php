...
$sql = ",SUM(Target_2023_24) AS Target_2023_24,SUM(P_Target) AS P_Target,SUM(A_c_Opened) AS A_c_Opened,SUM(A_c_Closed) AS A_c_Closed,SUM(Net_Acs) AS Net_Acs,SUM(Cert_Issued) AS Cert_Issued,SUM(Cert_Disch) AS Cert_Disch,SUM(Net_Cert) AS Net_Cert,SUM(MSSC_Opened) AS MSSC_Opened,SUM(Msscclosed) AS Msscclosed,SUM(Net_mssc) AS Net_mssc,SUM(Total_Acs) AS Total_Acs,SUM(Total_closed) AS Total_closed,SUM(Total_Net) AS Total_Net,(SUM(Total_Acs)/SUM(Target_2023_24))*100 AS `%age`,(SUM(Total_Acs)/SUM(P_Target))*100 AS `%age1`, Remarks,AO_Name,Sub_Division,Division FROM posb_karnataka1_23_24 WHERE 1";
...
if ($result->num_rows > 0) {
    ...
    echo "<table border='2'>
        ...
        <tr class='sticky-row'>
            ...
            <th data-backendName='Target_2023_24'>Annual</br>Target</th>
            ...
            <th data-backendName='Net_mssc'>Net <br />MSSC</th>
             <th data-backendName='Total_Acs'>Total A/cs <br />opened </br> (Incl Cert)</th>
             <th data-backendName='Total_closed'>Total A/cs <br />Closed </br>(Incl Cert)</th>
             <th data-backendName='Total_Net'>Total Net <br />A/cs </br>(Incl Cert)</th>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>{$row["Total_Acs"]}</td>";
        echo "<td>{$row["Total_closed"]}</td>";
        echo "<td>{$row["Total_Net"]}</td>";
        ...
        if (is_numeric($row["Target_2023_24"])) {
            $sums["Target_2023_24"] += (int) $row["Target_2023_24"];
        }
        ...
        if (is_numeric($row["Total_Acs"])) {
            $sums["Total_Acs"] += (int) $row["Total_Acs"];
        }
        if (is_numeric($row["Total_closed"])) {
            $sums["Total_closed"] += (int) $row["Total_closed"];
        }
        if (is_numeric($row["Total_Net"])) {
            $sums["Total_Net"] += (int) $row["Total_Net"];
        }
        if (is_numeric($achievment)) {
            $sums["Achievment"] = 0;
		    		if($sums["Target_2023_24"]) {
		    			$sums["Achievment"] = $sums["Total_Acs"] / $sums["Target_2023_24"];
		    		}
		    		$sums["Achievment"] *= 100;
        ...
        if (is_numeric($achievment1)) {
            $sums["Achievment1"] = 0;
		    		if($sums["P_Target"]) {
		    			$sums["Achievment1"] = $sums["Total_Acs"] / $sums["P_Target"];
		    		}
